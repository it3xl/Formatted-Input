...
namespace It3xl.Test.MoneyField.Silverlight
{
	...
	public class DelKeyTest : SilverlightTest
	{
		...
		public void LeadingZeros()
		{
			...
			beforeInput = "|90 623.25".ToSpecificValue(out beforeInputCaretPosition);
			input = "|0 623.25".ToSpecificValue(out inputCaretPositionRef);
			_scaffold.DoubleNullableMoneyTexBox.Converter.TestProcess(FocusState.Gotten, input, beforeInput, beforeInputCaretPosition, out formatteValueOut, ref inputCaretPositionRef);
			Assert.IsTrue(formatteValueOut == "|623.25".ToSpecificValue(out expectedCaretPosition));
			Assert.IsTrue(inputCaretPositionRef == expectedCaretPosition);
		}
		...
		public void DigitBeforeGroupSeparatorAtStartDeletion()
		{
			...
			beforeInput = "|1 846.72".ToSpecificValue(out beforeInputCaretPosition);
			input = "| 846.72".ToSpecificValue(out inputCaretPositionRef);
			_scaffold.DoubleNullableMoneyTexBox.Converter.TestProcess(FocusState.Gotten, input, beforeInput, beforeInputCaretPosition, out formatteValueOut, ref inputCaretPositionRef);
			Assert.IsTrue(formatteValueOut == "|846.72".ToSpecificValue(out expectedCaretPosition));
			Assert.IsTrue(inputCaretPositionRef == expectedCaretPosition);

			beforeInput = "|1 234 567.72".ToSpecificValue(out beforeInputCaretPosition);
			input = "| 234 567.72".ToSpecificValue(out inputCaretPositionRef);
			_scaffold.DoubleNullableMoneyTexBox.Converter.TestProcess(FocusState.Gotten, input, beforeInput, beforeInputCaretPosition, out formatteValueOut, ref inputCaretPositionRef);
			Assert.IsTrue(formatteValueOut == "|234 567.72".ToSpecificValue(out expectedCaretPosition));
			Assert.IsTrue(inputCaretPositionRef == expectedCaretPosition);
		}
